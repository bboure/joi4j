...
describe('neo4jDate', () => {
    ...
    it('should accept a Neo4j Date', async () => {
        const input = neo4j.types.Date.fromStandardDate(new Date('2019-01-01'));
        const result = await validator.validate(input, validator.neo4jDate());
        expect(result).toBeInstanceOf(neo4j.types.Date);
    ...
    it('min should work by value', async () => {
        const input = neo4j.types.Date.fromStandardDate(new Date('2019-01-01'));
        const same = neo4j.types.Date.fromStandardDate(new Date('2019-01-01'));
        const before = neo4j.types.Date.fromStandardDate(new Date('2018-12-31'));
        
        const resultSame = await validator.validate(input, validator.neo4jDate().min(same));
        expect(resultSame).toBeInstanceOf(neo4j.types.Date);
        expect(resultSame.toString()).toEqual(input.toString());
        
        const resultGreater = await validator.validate(input, validator.neo4jDate().min(before));
        expect(resultGreater).toBeInstanceOf(neo4j.types.Date);
    ...
    it('max should work by value', async () => {
        const input = neo4j.types.Date.fromStandardDate(new Date('2019-01-01'));
        const same = neo4j.types.Date.fromStandardDate(new Date('2019-01-01'));
        const after = neo4j.types.Date.fromStandardDate(new Date('2019-01-02'));
        
        const resultSame = await validator.validate(input, validator.neo4jDate().max(same));
        expect(resultSame).toBeInstanceOf(neo4j.types.Date);
        expect(resultSame.toString()).toEqual(input.toString());
        
        const resultGreater = await validator.validate(input, validator.neo4jDate().max(after));
        expect(resultGreater).toBeInstanceOf(neo4j.types.Date);
        expect(resultGreater.toString()).toEqual(input.toString());
    });
