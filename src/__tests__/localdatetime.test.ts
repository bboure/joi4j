...
describe('neo4jLocalDateTime', () => {
  describe('base', () => {
    ...
    it('should convert Js Date to neo4j.LocalDateTime', async () => {
      ...
      const neoLocalDateTimeNow = neo4j.types.LocalDateTime.fromStandardDate(
        dnow,
      );
    ...
    it('should throw an error if invalid neo4j.LocalDateTime', async () => {
      ...
      try {
        ...
        expect(error.details[0].message).toEqual(
          '"value" must be a valid LocalDateTime',
        );
      }
    });
  ...
  describe('min', () => {
    it('should work by value', async () => {
      ...
      const past = neo4j.types.LocalDateTime.fromStandardDate(
        new Date(now - 1000),
      );
    ...
    it('should work by reference', async () => {
      ...
      const to = neo4j.types.LocalDateTime.fromStandardDate(
        new Date(now + 1000),
      );
      await expect(
        validator
    ...
    it('should throw an error by value', async () => {
      ...
      const now = Date.now();
      const dnow = neo4j.types.LocalDateTime.fromStandardDate(new Date(now));
      const future = neo4j.types.LocalDateTime.fromStandardDate(
        new Date(now + 1000),
      );
      try {
